...
const int START = 3;
const int END = 4;
const int VISITED = 6; // Dodatkowy stan: odwiedzone pole
...
void drawGrid(SDL_Renderer* renderer, int grid[GRID_SIZE][GRID_SIZE]) {
    for (int y = 0; y < GRID_SIZE; ++y) {
        for (int x = 0; x < GRID_SIZE; ++x) {
            ...
            switch (grid[y][x]) {
            case OBSTACLE:
                SDL_SetRenderDrawColor(renderer, 255, 105, 180, 255);  // Różowy (Hot Pink)
                break;
            case PATH:
                SDL_SetRenderDrawColor(renderer, 199, 21, 133, 255);   // Ciemny różowy (Deep Fuchsia)
                break;
            case START:
                SDL_SetRenderDrawColor(renderer, 255, 20, 147, 255);  // Głębszy róż (Deep Pink)
                break;
            case END:
                SDL_SetRenderDrawColor(renderer, 219, 112, 147, 255);  // Bladoróżowy (Pale Violet Red)
                break;
            case VISITED:
                SDL_SetRenderDrawColor(renderer, 255, 192, 203, 255);  // Delikatny róż (Pink)
                break;
            default:
                SDL_SetRenderDrawColor(renderer, 255, 240, 245, 255);  // Bardzo jasny róż (Lavender Blush)
                break;
            }
